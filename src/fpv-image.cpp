...
{
    //Init ROS
    ros::init(argc,argv,"mambo_video", ros::init_options::AnonymousName);
    ...
    ros::Subscriber live_sub = n.subscribe("mambo_live", 15, cb_live);
    ...
    //Advertise img topic
    img_pub = it.advertise("mambo_out",100);

    //Get Mambo livestream url
    std::string mambo_url;
    n.param<std::string>("live_url", mambo_url, "rtsp://192.168.99.1/media/stream2");
    ...
    //Open video stream
    if(avformat_open_input(&format_ctx, mambo_url.c_str(), NULL, NULL) !=0)
    {
        return EXIT_FAILURE;
    }
